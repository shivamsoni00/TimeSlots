		...
		for (let i = 0; i < timeArr.length; i++) {
			...
			while (
				...
				if (
					...
					slotsArray.push({
						timeSlotStart: new Date(_startSlot).toLocaleTimeString(
							"en-US",
							{
								hour: "numeric",
								minute: "numeric",
				...
				_tempSlotStartTime = _endSlot.setMinutes(_endSlot.getMinutes());
			}
		}
					...
					{timeSlots &&
						timeSlots.map((slot, index) => (
							<div
								key={index}
								className={`slot ${
									index === Number(mainpara) ? "slot-red" : ""
								}`}
							>
